export const simplex3DGLSL = /* glsl */ `
...
void main() {
  ...
  vec3 displaced = position + normal * (baseNoise - crackEffect);
  
  vec4 worldPos = modelMatrix * vec4(displaced, 1.0);
  vec3 viewDir = normalize(cameraPosition - worldPos.xyz);
  vFresnel = pow(1.0 - max(dot(normal, viewDir), 0.0), 3.0);
...
void main() {
  vec3 baseColor = vec3(0.19, 0.51, 0.96);

  float hue = vNormal.x * 3.0 + vNormal.y * 3.0 + uTime * 0.15;
  vec3 rainbow = vec3(
  ...
  float crackDarken = 1.0 - uPressure * 0.2;
  vec3 finalColor = (baseColor + rainbow * vFresnel * 0.7) * crackDarken;
  finalColor += vec3(1.0) * vFresnel * 0.4;
...
void main() {
  ...
  float stretch = smoothstep(2.0, 0.0, toLen) * uPressure * 0.55;
  vec3 stretchEffect = normalize(toPointer + vec3(0.0001)) * stretch;

  vec3 displaced = position + normal * baseNoise + stretchEffect;
  csm_Position = displaced;
}
`;
